...
public class Problem_03_CardsInLine {
	...
	public static int f(int[] arr, int i, int j) {//先拿的
		...
		return Math.max(arr[i] + s(arr, i + 1, j), arr[j] + s(arr, i, j - 1));//先拿的加上后拿的价值最大
	}

	public static int s(int[] arr, int i, int j) {//后拿的
		if (i == j) {//只剩下一个肯定被先拿的拿走了
			return 0;
		}
		return Math.min(f(arr, i + 1, j), f(arr, i, j - 1));//先拿的肯定是拿大的，所以后拿只能拿到先拿中较小的数
	}
